...
public class Transfer extends Transaction{
	...
	public int execute() {
		...
		if (status != TRANSFER_CANCELED) { // cek apakah idxamount bukan canceled (6) 
			...
			availableBalance = bankDatabase.getAvailableBalance(currAcc);
			   ...
			   // some validation
			if (!isBalanceEnoughTransfer(availableBalance,idxamount)) { // cek saldo apakah masih mencukupi (v3)
				status = BALANCE_NOT_ENOUGH;
			}else {
				if(currAcc == destAcc) { // cek account number same with destination account number (V4)
					status = SAME_ACCOUNT_NUMBER;
				}else if (!bankDatabase.isAccountExist(destAcc)) { // cek destination account is not exist (V5)
					status = ACCOUNT_NOT_REGISTERED;
				}else if(!bankDatabase.isAccountActive(destAcc)) { // cek destination account is active (V6)
					status = DEST_ACCOUNT_BLOCKED;
	...
	private boolean isBalanceEnoughTransfer(double balance, int amount) {
		if ((balance-amount)>= MIN_AMOUNT_BALANCE) {
			return true;
		}else {
			return false;
		}

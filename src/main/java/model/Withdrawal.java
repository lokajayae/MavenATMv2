...
public class Withdrawal extends Transaction {
   ...
   public int execute() {
	   ...
	   if (idxamount != WITHDRAWAL_CANCELED) { // cek apakah idxamount bukan canceled (6) 
		   ...
		   if(!cashDispenser.isSufficientCashAvailable(idxamount)) { // V4
			   // cek dulu uang yang ada di dispenser ATM apakah masih ada?
			   status = CASHDISPENSER_NOT_ENOUGH ;
		   }else if (!isBalanceEnoughWithdraw(availableBalance, idxamount)) { // cek saldo apakah masih mencukupi (V5)
	...
	private boolean isBalanceEnoughWithdraw(double balance, int amount) {

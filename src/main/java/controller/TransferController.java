...
public class TransferController extends TransactionController{
		...
		do {
			...
			while (statusTransfer <= 0) {
				switch (statusTransfer) {
					case TRANSFER_ERROR_POSITIVE_AMOUNT: // cek amount bernilai negatif
					...
					case TRANSFER_ERROR_RANGE_AMOUNT: // cek range value amount dengan batasan limit value
						screen.displayMessageLine("Invalid selection. Try Again. the nominal amount transfer does not match the valid range "
			...
			switch (statusProcess) {
				...
				case ACCOUNT_NOT_REGISTERED:
					screen.displayMessageLine("Destination account not found");
					break;
				case DEST_ACCOUNT_BLOCKED:
	...
	private void displayMenuOfAmounts() {
		...
		switch (inputMenu) {
			case 1: // do transfer amount
	        	...
	        	if (!positifValidTransferAmount(inputAmount)) { // V1, amount harus positif value
	        		statusTransfer = TRANSFER_ERROR_POSITIVE_AMOUNT;
	...
	private boolean positifValidTransferAmount(int amount) {
		if(amount > 0) {
			return  true;
		}else {
			return false;
